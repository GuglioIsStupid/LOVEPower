...
int extloader(lua_State *L)
{
    ...
    // Tokenize the filename (replace dots with slashes for path and underscores for function names)
    ...
    void *handle = nullptr;  // Use void* for SDL2 dynamic library handles
    auto *inst = instance();
    std::string loaderror = "";

    // Loop through C require paths
    for (const std::string &el : inst->getCRequirePath())
    {
        for (const char *ext : library_extensions)
        {
            std::string element = el;
            replaceAll(element, "??", tokenized_name + ext);  // Add extension to filename
            replaceAll(element, "?", tokenized_name);  // Replace with tokenized filename

            Filesystem::Info info = {};
            if (!inst->getInfo(element.c_str(), info) || info.type == Filesystem::FILETYPE_DIRECTORY)
                continue;

            // Resolve the full path and load the shared object
            std::string filepath = inst->getRealDirectory(element.c_str()) + LOVE_PATH_SEPARATOR + element;
            /* handle = SDL_LoadLibrary(filepath.c_str()); */ // loadlivrary not in sdl2
    ...
    // Look for both "loveopen_" and "luaopen_" function names
